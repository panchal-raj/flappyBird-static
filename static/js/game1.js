document.addEventListener('DOMContentLoaded', function() {
    ...
    function isStarOverlappingPipe(starX, starY, starWidth, starHeight) {
        // Define the star's boundaries
        ...
        for (const pipe of gameState.pipes) {
            // Define the pipe's boundaries
            const pipeLeft = pipe.x;
            const pipeRight = pipe.x + pipe.width;
            
            // Only check horizontal overlap if there is any
            if (starRight >= pipeLeft && starLeft <= pipeRight) {
                // Check if star overlaps with top pipe
                if (starTop <= pipe.topHeight && starBottom >= 0) {
                    return true;
                }
                
                // Check if star overlaps with bottom pipe
                if (starBottom >= pipe.bottomY && starTop <= canvas.height) {
                    return true;
                }
                
                // Add buffer zone to avoid spawning stars too close to pipe gap
                const bufferZone = 20;
                if (starTop >= pipe.topHeight - bufferZone && starBottom <= pipe.bottomY + bufferZone) {
                    return true;
                }
            }
        }
    ...
    function handlePipesMode() {
        ...
        if (gameState.pipes.length === 0 || 
            ...
            canvas.width - gameState.pipeDistance - gameState.pipeWidth) {  // Add pipe width to ensure proper spacing
    ...
    function createStar() {
        ...
        const starWidth = isBigStar ? 50 : 30;
        const starHeight = isBigStar ? 50 : 30;
        ...
        if (gameSettings.mode === 'pipes-and-stars') {
            ...
            while (attempts < maxAttempts) {
                ...
                if (lastPipe) {
                    ...
                    // Fix: Pass correct dimensions to isStarOverlappingPipe
                    conflictsWithPipe = isStarOverlappingPipe(x, y, starWidth, starHeight);
                }
